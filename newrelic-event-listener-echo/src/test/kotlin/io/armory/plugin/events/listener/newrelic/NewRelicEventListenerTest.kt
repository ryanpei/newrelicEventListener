...
class NewRelicEventListenerTest : JUnit5Minutests {
    ...
    fun tests() = rootContext {
        ...
        test("process pipeline event") {
            ...
            val eventListener = HttpMockableNewRelicEventListener(okHttpClient, log)
            ...
            verify(exactly = 1) {
                okHttpClient.newCall(
                        withArg {
                            ...
                            val actualJson = mapper.readValue(newRelicEvent.text, Map::class.java)
        ...
        test("log error when processing execution event fails") {
            ...
            verify(exactly = 1) {
                log.error(
                        withArg {
                            expectThat(it).startsWith("NewRelic event listener failed with response: ")
                        }
                )
            }
        }
